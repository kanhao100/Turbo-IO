...
- ASR 网络错误按 1、2、4 秒重连。重连期间最多缓存约 2 秒 PCM；只有溢出、丢包或不可恢复异常才写 gap。认证、额度和配置错误不盲目重试。
...
不使用真实服务 Key、用户音频或 Apple 签名凭据。设备 CI 输出未签名 0.3.5 (11) IPA；IPA 文件名与 Actions 产物名包含 `v0.3.5-build11`，仍需用户自己的签名才能安装。真实 A3 持续到达、第二次双击事件、锁屏后台、休眠重连和 30 分钟运行仍必须在目标眼镜上验收，CI 编译不等于这些已通过。
