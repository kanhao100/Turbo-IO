...
字幕与显示共用同一个协议 SID，不开启另一条 AI 语音会话。音频按 SID、设备、时间和序号检查；重复/乱序丢弃，缺口显式记录。libopus 解码到 16kHz mono（库内下混），不把 stereo 的单侧误当整体；不能解码的包停止并提示，而不伪造音频成功。
...
不使用真实服务 Key、用户音频或 Apple 签名凭据。设备 CI 输出未签名 0.3.0 (6) IPA，仍需用户自己的签名才能安装。
